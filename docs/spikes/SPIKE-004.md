...
- Later on 2026-08-13, the user completed the blocking exact-duplicate relay check with extension version `0.1.1`. A fresh operation reached terminal authenticated status `1`; before explicit resend, Chrome reported two locally accepted invoke deliveries and Android's persisted redacted side-effect count increased exactly once from `2` to `3`. Selecting `Resend exact completed action` then increased Chrome's durable accepted invoke-delivery count from `2` to `3` and authenticated result-delivery count from `10` to `11`, while Android's side-effect count remained `3`. The explicit resend re-used the exact canonical invoke and business idempotency key under a fresh envelope tuple, and Android returned its cached canonical result without a second side effect. Result-delivery count is intentionally not treated as operation count while end-to-end ACK is absent. Restart and interrupted-network variants remain separate evidence work.
